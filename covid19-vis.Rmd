...
Below is the most important plot. We want to be like S. Korea or lower.  This is a comparison of growth per capita (on a logarithmic scale), where  "day 1" is approx 0.0004% of the country's population.
In reality, we want to know what percentage of hospital beds have been taken up, and so
population is just a proxy. The dashed grey lines show epidemic growth when the fraction of infected population doubles every 2 days, or 3 days, etc.
```{r}
...
comparison<- filter(covid,type %in% c('confirmed'))  %>%
	...
	filter( region%in% c('Italy','US','S.Korea','Spain')) %>%
...
comp.pc<-comparison %>% filter(val/pop>0.00004) %>%
	group_by(region) %>%
	mutate(day=row_number()-1,valpc=val/pop)

labels<- comp.pc %>% group_by(region) %>% filter(day==max(day))


template<- expand.grid(day=0:25,rate=c(2,3,4,5,10)) %>%
	as.tbl %>%
	mutate(region=paste(rate),valpc=0.00004*2**(day/rate))

gr.labels=tibble(text=c('every\n2 days','every\n3 days','every\n4 days',
												'every\n5 days','every\n10 days'),
								 x=c(10,16,23,23,15),
								 y=c(.0018,.0018,.0018,.0007,.0001))
...
ggplot(comp.pc)+
	...
	geom_text(data=gr.labels,aes(label=text,group=NA,x=x,y=y),color='grey')+
	...
	coord_cartesian(ylim=c(0.00004,.002))+
	labs(y='confirmed cases per capita',x='Days since 0.004% of the population infected')
...
ggplot(tmp)+aes(date,delta_pos/(delta_pos+delta_neg))+
	geom_point()+geom_line()+
	scale_y_continuous(limits=c(0,NA),label=percent,breaks = pretty_breaks())+
			scale_x_date(breaks=pretty_breaks())+
