...
ma.orig<-tibble(date=seq(as.Date("2020-03-03") ,as.Date("2020-03-17"),by='1 day'),
								val=c(1,2,7,8,13,28,41,88,95,108,123,138, 164,197,218)) %>%
...
compare<- filter(covid,type %in% c('confirmed'))  %>%
	...
	mutate(pop=ifelse(region=='Italy',  60462000,ifelse(region=='US',  331003000,ifelse(region=='S.Korea',  51269000))))
...
ggplot(compare)+
	...
	theme(legend.position = "none")+
	geom_line()+geom_point()+
	geom_text_repel(data=labels,aes(label=region),color='black',direction='y')+
