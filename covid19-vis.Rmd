...
tmp=group_by(covid,region,type,date) %>% 
	filter(val>0,region!='China',date>='2020-03-01') %>%
	summarize(val=sum(val)) %>%
	group_by(region) %>%
	filter(max(val)>=1)
...
ggplot(tmp)+theme_gray(12)+
	...
	geom_text(data=labels %>%	 
						...
						hjust=0,
						aes(label=region,x=date+.5),color='black')
######################
```
...
```{r MA-fit, fig.height=4,fig.width=4}
...
ggplot()+
	geom_line(data=fit,aes(x=date,y=val,color='fit'),linetype=2)	+
	geom_point(data=fit,aes(x=date,y=val,color='fit'))	+
	geom_point(data=ma.orig,aes(date,val,color='actual'))+
	scale_y_continuous(breaks=pretty_breaks(5),label=comma,
										 sec.axis = dup_axis())+
	scale_x_date(breaks=pretty_breaks())+
	theme(legend.position = c(.2,.9))+
	labs(y="confirmed cases in MA",x="")

```

# Focus on CA
```{r CA, fig.height=4,fig.width=4}
...
ggplot()+
	...
	labs(y="confirmed cases in CA",x="")
...
ggplot(rate)+
	...
	theme(legend.position='none')+
	geom_text(data=labels,aes(label=region,x=date+2),hjust=0,color='black')
