...
This plot shows the spread of the virus over time across the world. We want the number of active cases  to drop to 0, the deaths to stay as low as possible obviously, and the recovered cases (blue) to climb high. Confirmed is a cumulative count of everyone that has tested positive. 
...
MA is performing an increasing (but still inadequate) number of tests per day. 


```{r ma-testing, fig.height=3,fig.width=5}
ma.testing<-select(ma.gov,date,contains("Lab_")) %>%
	gather(var,val,-date) %>% 
	na.omit %>%
...
# 
# ggplot(ma.testing)+
# 	aes(date,delta,group=result,color=result)+
# 	geom_point()+geom_line()+
# 	scale_y_log10(limits=c(1,NA))+
# 	labs(x='',y='Daily test results in MA')
...
````{r states,fig.height=9,fig.width=5}
ct<- 
...
ggplot(tmp %>% filter(state %in% labels$state) %>% filter(!is.na(val)) )+
	...
	geom_text(data=labels,
						...
						color='black',
						position = position_jitter(width = 2,height = .1,seed=1)
	)+
...
ggplot(tmp)+
	...
	geom_text(aes(label=confirmed+negative,x=round(rate+.15,1)),hjust=1)+
